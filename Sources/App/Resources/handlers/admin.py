...
def handle_get_backup_config(handler):
    ...
    Docker 环境下: 备份目录由 METER_BACKUP_DIR 环境变量控制，
...
def handle_put_backup_config(handler):
    """PUT /api/admin/backup-config → {ok: false, error: "Docker 环境下备份目录由环境变量 METER_BACKUP_DIR 控制"}

    Docker 环境下备份目录由 docker-compose.yml 中的 METER_BACKUP_DIR 环境变量控制，
    不支持通过 API 动态修改。
    """
    send_json(handler, 200, {
        "ok": False,
        ...
        "data_dir": str(Path(os.environ.get("METER_DATA_DIR", "/data"))),
    })

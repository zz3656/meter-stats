...
def handle_get_backup_status(handler):
    ...
    if backup_dir.exists():
        ...
        for f in sorted(backup_dir.rglob("*.zip"), reverse=True):
            if f.is_file():
                # 从文件名提取备份时间戳
                # 格式: linclub-backup-20250101_120000.zip → 20250101_120000
                name = f.name
                try:
                    ts = name.replace("linclub-backup-", "").replace(".zip", "")
                except Exception:
                    ts = name
...
def handle_get_backup_download(handler):
    """GET /api/admin/backup-download?zip_name=linclub-backup-20250101_120000.zip → 下载 .zip 文件"""
    from storage import get_data_dir
    from urllib.parse import parse_qs, urlparse
    ...
    if not zip_name:
        ...
        if dir_name:
            zip_name = f"linclub-backup-{dir_name}.zip"
        else:
